...
#if UNITY_EDITOR
        public List<Vector3> previousPositions = new List<Vector3>();   //어택포인트의 시작 위치
#endif
    }
        ...
        private void FixedUpdate()
        {
            if (m_InAttack)
            {
                for (int i = 0; i < attackPoints.Length; i++)
                {
                    ...
                    Vector3 worldPos = pts.attackRoot.position + pts.attackRoot.TransformVector(attackPoints[i].offset);
                    //검이 휘둘리는 방향
